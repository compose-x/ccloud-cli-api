...
coverage: ## check code coverage quickly with the default Python
	coverage run --source ccloud_cli_api -m pytest
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/ccloud_cli_api.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ ccloud_cli_api
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
conform	: ## Conform to a standard of coding syntax
	isort --profile black ccloud_cli_api
	black ccloud_cli_api tests
	find ccloud_cli_api -name "*.json" -type f  -exec sed -i '1s/^\xEF\xBB\xBF//' {} +
...
python38	: clean dist
			...
			-c "pip install --no-cache-dir /opt/dist/*.whl -t /opt/layer"
